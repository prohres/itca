...
resource "aws_cloudwatch_log_group" "kantor" {
  name = "kantor"
...
resource "aws_ecr_repository" "kantor_app" {
...
data "template_file" "web_task" {
  ...
  vars {
    image           = "${aws_ecr_repository.kantor_app.repository_url}"
    log_group       = "${aws_cloudwatch_log_group.kantor.name}"
  }
}
...
resource "aws_alb" "alb_kantor" {
  name            = "${var.environment}-alb-kantor"
...
resource "aws_alb_listener" "kantor" {
  load_balancer_arn = "${aws_alb.alb_kantor.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_kantor_web_cpu_utilization_high"
